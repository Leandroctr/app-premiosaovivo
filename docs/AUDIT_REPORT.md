...
- **M-9 — corrigido:** removida a policy anon `Allow public read app_settings`
  ...
  `source: "database"` e tenant `pwa.app-premiosaovivo.com`.
...
Este bloco foi propagado semanticamente para o worktree Prêmios ao Vivo de
segurança, a partir do HEAD `de0b4a7931ffc32677ce737282ef28addf145fce`. Não
houve migration, DDL, alteração de configuração do bucket, push ou deploy. As suítes reais criaram
...
No Prêmios ao Vivo passaram: instalação reproduzível com `npm ci --no-audit --no-fund`,
